...
const NewMeeting = () => {
  ...
  useEffect(() => {
    ...
    socket.on("new-meeting", (data) => {
      ...
      if (data.status == "success") {
        ...
        showToast("Error in creating meeting", "error");
      }
    });
  ...
  useEffect(() => {
    ...
    inputs.forEach((input) => {
      input.maxLength = 1;
      input.addEventListener("input", handleCodeChange);
      input.addEventListener("keydown", handleCodeDelete);
    });

    return () => {
      inputs.forEach((input) => {
        input.removeEventListener("input", handleCodeChange);
        input.removeEventListener("keydown", handleCodeDelete);
      });
    };
  }, []);
    ...
    <div className={styles.main}>
      ...
      <div className={styles.right}>
        <div className={styles.joinMeetingCard}>
          ...
          <div className={styles.joinMeetingButton} onClick={handleCodeSubmit}>
            <p>JOIN</p>
            <CiCircleCheck />
          </div>
        </div>
        ...
        <div className={styles.newMeetingCard} onClick={createNewMeeting}>
